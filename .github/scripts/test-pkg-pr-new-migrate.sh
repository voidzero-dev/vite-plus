...
# Remove the existing lockfile and node_modules so migrate's reinstall resolves
# from scratch. A stale pre-migrate lockfile can keep an optional-peer copy of
# vite-plus pinned to an older published version (e.g. a nested oxlint's
...
echo
echo "Clearing lockfiles and node_modules in $project_dir for a clean migrate install"
...
find "$project_dir" -type d -name node_modules -prune -exec rm -rf {} + 2>/dev/null || true
