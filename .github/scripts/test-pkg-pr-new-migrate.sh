...
# Install the project's committed dependencies before migrating, as a user
# upgrading would have them. `vp migrate` reads the original Vitest version from
# the lockfile or node_modules, and blocks the Vitest 5 migration without them.
# The install runs without the preview overrides so it reproduces the original
# dependency tree. A project without a usable lockfile falls back to a regular
# install.
echo
echo "Installing the committed dependencies in $project_dir before migrating"
install_status=0
(cd "$project_dir" && env -u VP_VERSION -u VP_OVERRIDE_PACKAGES "$vp_bin" install --frozen-lockfile) ||
  install_status=$?
if [ "$install_status" -ne 0 ]; then
  echo "warning: frozen install failed (exit $install_status); retrying without --frozen-lockfile" >&2
  (cd "$project_dir" && env -u VP_VERSION -u VP_OVERRIDE_PACKAGES "$vp_bin" install) ||
    echo "warning: could not install the committed dependencies; migrating anyway" >&2
fi
