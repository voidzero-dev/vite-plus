...
pub(super) fn parse_args<T>(bin_name: &'static str, argv: Vec<String>) -> ParseResult<T>
where
    T: Args + FromArgMatches,
{
    let command = T::augment_args(Command::new(bin_name));
    let parsed = command
        .try_get_matches_from(iter::once(bin_name.to_owned()).chain(argv))
        .and_then(|mut matches| T::from_arg_matches_mut(&mut matches));

    match parsed {
        Ok(value) => ParseResult::Ok(value),
        Err(error) if error.kind() == ErrorKind::DisplayHelp => ParseResult::Help,
