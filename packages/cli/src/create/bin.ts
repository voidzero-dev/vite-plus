  ...
  sections: [
      ...
      rows: [
        {
          ...
          description: [
            ...
            `- Org scope: ${accent('@acme')} → picker from ${accent('@acme/create')}'s ${accent('vp.templates')} manifest`,
            `- Org entry: ${accent('@acme/web')} → manifest entry "web" (falls back to ${accent('@acme/create-web')})`,
    ...
    },
    {
      ...
      lines: [
        ...
        `  ${accent('vp create @acme')} ${muted('# interactive picker')}`,
        `  ${accent('vp create @acme/web')} ${muted('# direct selection')}`,
      ],
    },
  ],
});
