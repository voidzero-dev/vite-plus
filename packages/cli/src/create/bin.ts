  ...
  sections: [
      ...
      rows: [
        {
          ...
          description: [
            ...
            `- Org scope: ${accent('@nkzw')} → picker from ${accent('@nkzw/create')}'s ${accent('vp.templates')} manifest`,
            `- Org entry: ${accent('@nkzw/web')} → manifest entry "web" (falls back to ${accent('@nkzw/create-web')})`,
    ...
    },
    {
      ...
      lines: [
        ...
        '',
        `  ${muted('# Pick from an org that publishes @scope/create with vp.templates')}`,
        `  ${accent('vp create @nkzw')} ${muted('# interactive picker')}`,
        `  ${accent('vp create @nkzw/web')} ${muted('# direct selection')}`,
      ],
    },
  ],
});
