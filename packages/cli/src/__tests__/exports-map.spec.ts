...
/**
 * Migration rewrites Oxlint JS-plugin authoring imports to `vite-plus/lint/plugins`
 * and `vite-plus/lint/rule-tester` (see the `rewrite-oxlint-plugin-*` rules in
 * `import_rewriter.rs` and `rewriteVitePlusImportSpecifier` in
 * `oxlint-plugin.ts`). The whole point of that rewrite is that a user's plugin
 * file reaches the API through `vite-plus` instead of pinning its own
 * `@oxlint/plugins`, so these entrypoints MUST stay resolvable and MUST keep
 * re-exporting the upstream surface, otherwise every migrated plugin breaks at
 * the moment `vp lint` tries to load it.
 */
describe('Oxlint JS-plugin authoring entrypoints', () => {
  ...
  it('exposes RuleTester from vite-plus/lint/rule-tester', async () => {
    const ruleTester = await import('vite-plus/lint/rule-tester');
