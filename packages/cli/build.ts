...
async function buildNapiBinding() {
  ...
  const rolldownDtsHeader = process.env.RELEASE_BUILD
    ? (await import('../../rolldown/packages/rolldown/package.json', { with: { type: 'json' } }))
        .default.napi.dtsHeader
    : '';

  // Always include eslint-disable comment for generated binding files
  const eslintDisableHeader = '/* auto-generated by NAPI-RS */\n/* eslint-disable */\n';

  if (rolldownDtsHeader) {
    passedInOptions.dtsHeader = `${eslintDisableHeader}type BindingErrorsOr<T> = T | BindingErrors;\ntype FxHashSet<T> = Set<T>;\ntype FxHashMap<K, V> = Map<K, V>;\n${rolldownDtsHeader}`;
  } else {
    passedInOptions.dtsHeader = eslintDisableHeader;
  }
