...
async function readNativeBuildTime(): Promise<string | undefined> {
...
async function resolveToolchainNode(
  ...
  switch (node.versionSource.type) {
    case 'cli-package':
      version = cliVersion;
      break;
    case 'core-package':
    ...
    case 'core-bundled': {
      const bundledVersions = (corePkg as { bundledVersions?: Record<string, string> })
        .bundledVersions;
      version = bundledVersions?.[node.versionSource.key] ?? '';
      break;
    }
    case 'npm-dependency':
      version = await readPackageVersion(
        join(projectDir, 'node_modules', node.versionSource.package, 'package.json'),
        `${node.versionSource.package}/package.json`,
      );
      break;
    case 'cargo': {
      const pkg = resolveCargoPackage(cargoMetadata, node.versionSource.package);
      revision = sourceRevision(pkg.source);
      if (node.versionSource.revision && !revision) {
        throw new Error(`Expected an exact source revision for Cargo package ${pkg.name}`);
      }
      if (node.versionSource.builtAt) {
        builtAt = buildTime;
      } else {
        version = pkg.version;
      }
...
async function syncToolchainExports() {
  ...
  const buildTime = await readNativeBuildTime();
  ...
  await writeFile(join(distDir, 'toolchain.json'), `${JSON.stringify(toolchain, null, 2)}\n`);
  await writeFile(
    join(distDir, 'toolchain.js'),
    `export const toolchain = ${JSON.stringify(toolchain, null, 2)};\nexport default toolchain;\n`,
  );
  await writeFile(
