    ...
    try {
        ...
        if (Test-Path $autoloadFile) {
            ...
            if ($existing -eq $content) {
                return [pscustomobject]@{
                    Status = "already"
                    Message = "already configured $(Format-DisplayPath -Path $autoloadFile)"
                }
            }
        }
        ...
        return [pscustomobject]@{
            Status = "true"
            Message = "updated $(Format-DisplayPath -Path $autoloadFile)"
        }
    } catch {
        Write-Warn "Could not configure Nushell automatically."
        return [pscustomobject]@{
            Status = "failed"
            Message = "failed $(Format-DisplayPath -Path $autoloadFile)"
        }
    }
}
