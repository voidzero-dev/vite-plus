...
get_latest_version() {
  local version
  version=$(curl -s "https://registry.npmjs.org/vite-plus-cli/latest" | grep -o '"version":"[^"]*"' | cut -d'"' -f4)
  if [ -z "$version" ]; then
...
main() {
  ...
  local binary_url="https://registry.npmjs.org/${package_name}/-/vite-plus-cli-${package_suffix}-${VITE_VERSION}.tgz"
...
EOF
  ...
  local main_url="https://registry.npmjs.org/vite-plus-cli/-/vite-plus-cli-${VITE_VERSION}.tgz"
