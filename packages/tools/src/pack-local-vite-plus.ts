// Pack the checkout's publishable Vite+ packages (vite-plus,
// @voidzero-dev/vite-plus-core, and the host native platform package) so a
// local npm registry can serve them at the checkout version. Shared by the
// snap-test harness (once-per-run pack) and local-npm-registry.ts --pack
// (standalone dev / e2e invocations).
//
