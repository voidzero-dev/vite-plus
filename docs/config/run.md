...
- **CI/CD:** `CI`, `VERCEL_*`, `NEXT_*`, `GITHUB_*`, `RUNNER_*`, `ACTIONS_ID_TOKEN_REQUEST_URL`, `ACTIONS_ID_TOKEN_REQUEST_TOKEN`. GitHub Actions tasks can request OpenID Connect (OIDC) tokens without extra `untrackedEnv` config. These variables do not affect the cache fingerprint.
