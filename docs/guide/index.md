...
const agentSetupUrl = `${__DOCS_ORIGIN__}/agent-setup/prompt.md`;
</script>
...
Copy this prompt into your coding agent to install or reuse the global `vp` CLI and install the official Vite+ skill globally. The skill teaches agents to prefer `vp` for new projects and package workflows before a project has local instructions.

<CopyPrompt />

You can <a :href="agentSetupUrl">view the setup instructions</a>, update the installed skill with `vp dlx skills update vite-plus --global --yes`, or remove it with `vp dlx skills remove vite-plus --global --yes`.

Project instructions generated by `vp create` and `vp migrate` complement the global skill with repository-specific guidance.
