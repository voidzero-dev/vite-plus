...
- `--approve-builds` approves and runs gated dependency build scripts without prompting (pnpm)
...
For security, pnpm does not run a dependency's build scripts (`install` / `postinstall`, e.g. native builds like `better-sqlite3`) until you approve them. When a template adds such a dependency directly, `vp create` surfaces it after installing instead of leaving the project in a half-built state:
...
Transitive build scripts you did not choose (e.g. `esbuild` pulled in by Vite) are left as pnpm's defaults and are not surfaced.
