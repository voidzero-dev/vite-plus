...
::: info
`vp test` always runs the built-in Vitest command. If your project also has a `test` script in `package.json`, run `vp run test` or `vpr test` when you want to run that script instead. See [Built-in Commands vs Scripts](/guide/run#built-in-commands-vs-scripts).
:::
