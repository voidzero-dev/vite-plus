...
- **Category B (JS Script Commands)**: `create`, `migrate`, `config`, `hooks`, `staged` — implemented in JavaScript. Rust uses `oxc_resolver` to find the project's local `vite-plus/dist/bin.js` and runs it with the managed Node.js runtime, falling back to the global installation's `dist/bin.js` when no local installation exists. The unified `bin.ts` entry point then loads the rolldown-bundled module for the command. `migrate` is the one exception to local-first resolution: `JsExecutor::delegate_migrate` compares versions and runs the global CLI instead when the project's local `vite-plus` is older than the global `vp`.
