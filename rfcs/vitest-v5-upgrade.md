...
| `@vitest/browser-webdriverio`                                                                 | Retain Vitest's optional peer metadata. Remove Vite+ provider exports and restore legacy imports to the [community package](https://github.com/vitest-community/vitest-webdriverio). Users manage provider versions and peers; migration preserves their dependency specifications. |
...
1. Verify the direct community import from `@vitest/browser-webdriverio@5.0.0` against Vitest `5.0.1`. Retain upstream optional peer metadata without Vite+ provider shims. Users select compatible provider versions; migration must preserve their declarations.
