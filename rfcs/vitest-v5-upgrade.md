...
The repository vendors Vite and Rolldown workspaces. Keep the root Vitest v5 pin when merging their catalogs during `sync-remote`, and align Vite's direct `@vitest/*` dependencies with that pin. Leave Rolldown's test dependency upgrades to its upstream maintainers. This upgrade does not require Rolldown test suites or their fixture packages in the root workspace.

Run Vite's Node suites with `pnpm test:vendored` after building Vite and `create-vite`. The test bridge preserves their v4 mock-history setting and checks nine Vite+ branding differences without editing the vendored test files. Keep upstream Vite's `dedupeInjectedDeps: false` setting so pnpm retains `file:` dependency placement for the SSR externalization and license tests.
...
12. Vendored Vite Node tests under the final synchronized dependency graph. Rolldown's Vitest upgrade and test suites are outside this release gate.
...
- The September 6 `pnpm test:vendored` run passed 1,904 tests and skipped 19 across Vite, Rolldown, Rolldown watch, and dev-server fixture suites. We have since limited this command to Vite's Node suites; Rolldown validation is outside the scope of this upgrade.
...
2. Does vendored Vite pass its Node suites with the synchronized Vitest v5 dependencies? Keep the Vite dependency alignment until upstream adopts the selected version. Rolldown's Vitest version does not block this release.
