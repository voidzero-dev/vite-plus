...
/// File name under [`VpDirs::state_dir`].
...
fn write_cache(cache_path: &vt_path::AbsolutePath, cache: &UpgradeCheckCache) {
    if let Ok(data) = serde_json::to_string(cache) {
        // Under the split layout the state dir may not exist yet (nothing
        // else creates it on a fresh install), so create it first; without
        // this the backoff cache never persists and every invocation
        // re-queries the registry.
        if let Some(parent) = cache_path.parent() {
            let _ = std::fs::create_dir_all(parent.as_path());
        }
...
pub async fn check_for_update() -> Option<UpgradeCheckResult> {
    let cache_path = VpDirs::state_dir().join(CACHE_FILE_NAME);
