...
//!
//! On-disk locations come from [`VpDirs`].
...
/// Main config file name under [`VpDirs::config_dir`].
...
fn config_file_path() -> AbsolutePathBuf {
    VpDirs::config_dir().join(CONFIG_FILE_NAME)
}

fn session_version_file_path() -> AbsolutePathBuf {
    VpDirs::state_dir().join(SESSION_VERSION_FILE)
}
...
pub async fn read_session_version() -> Option<String> {
    let path = session_version_file_path();
...
pub async fn resolve_version(cwd: &AbsolutePath) -> Result<VersionResolution, Error> {
    ...
    if let Some(session_version) = read_session_version().await {
        return Ok(VersionResolution {
            ...
            source_path: Some(session_version_file_path()),
            project_root: None,
            is_range: false,
        });
    }
...
pub async fn resolve_version_from_files(cwd: &AbsolutePath) -> Result<VersionResolution, Error> {
    ...
    if let Some(default_version) = config.default_node_version {
        ...
        return Ok(VersionResolution {
            ...
            source_path: if is_alias { None } else { Some(config_file_path()) },
            project_root: None,
            is_range,
        });
    }
...
pub async fn resolve_version_alias(
    ...
    result.map_err(|e| match e {
        Error::RuntimeDownload(
            vp_js_runtime::Error::SemverRange(_) | vp_js_runtime::Error::NoMatchingVersion { .. },
        ) => Error::Other(
            format!(
