...
pub async fn execute(cwd: AbsolutePathBuf) -> Result<ExitStatus, Error> {
    ...
    match &env_status {
        ...
        Some(EnvSourcingStatus::ShellOnly | EnvSourcingStatus::NotFound) => {
            ...
            print_ide_setup_guidance(&VpDirs::config_dir());
        }
    }
...
async fn check_path() -> bool {
    ...
    if bin_in_path {
        ...
        println!();
        print_path_fix(&VpDirs::config_dir());
        return false;
    }
...
fn find_in_path(name: &str) -> Option<std::path::PathBuf> {
    ...
    vp_command::resolve_bin(name, None, &cwd).ok().map(|p| p.into_path_buf())
}
...
async fn find_nearest_dev_engines_node_version(cwd: &AbsolutePathBuf) -> Option<vt_str::Str> {
    ...
    loop {
        if let Ok(content) = tokio::fs::read_to_string(current.join("package.json")).await
            && let Ok(pkg) = serde_json::from_str::<vp_shared::PackageJson>(&content)
...
async fn collect_dev_engines_findings(
    ...
    let Ok(pkg) = serde_json::from_str::<vp_shared::PackageJson>(&content) else {
        return findings;
    };
    ...
    if let Some(field) = package_manager_field {
        let is_supported = |name: &str| vp_pm_cli::PackageManagerType::from_name(name).is_some();
...
fn collect_malformed_entry_findings(
    ...
    for entry in entries {
        ...
        if let Some(on_fail) = obj.get("onFail").and_then(serde_json::Value::as_str)
            && vp_shared::OnFail::parse(on_fail).is_none()
        {
            findings.push(DevEnginesFinding::warn(
                "Spec",
                format!(
