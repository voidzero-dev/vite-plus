...
use vp_shared::{PrependOptions, VpDirs, exit_code_from_status, output, prepend_to_path_env};
...
pub async fn execute_vpx(args: &[String], cwd: &AbsolutePath) -> i32 {
    ...
    let dlx = vp_pm_cli::PackageManagerCommand::dlx(
        flags.packages,
        flags.shell_mode,
        flags.silent,
        positional,
    );
    match vp_pm_cli::dispatch(cwd, dlx).await {
