...
pub async fn dispatch(tool: &str, args: &[String]) -> i32 {
    ...
    if tool == "npm" {
        if let Some(parsed) = parse_npm_global_install(args) {
            ...
            if exit_code == 0 {
                let node_dir = VpDirs::js_runtime_dir().join("node").join(&*resolution.version);
...
pub(crate) async fn ensure_installed(version: &str) -> Result<AbsolutePathBuf, String> {
    let version_dir = VpDirs::js_runtime_dir().join("node").join(version);
...
mod tests {
    ...
    /// both A's dir (from bypass) and its own dir (from `VpDirs::bin_dir`), finding the real tool
    ...
    fn test_find_system_tool_cumulative_bypass_prevents_loop() {
        ...
        // Installation B also needs to filter install_b_bin (via VpDirs::bin_dir),
        // but VpDirs::bin_dir resolves this process's own bin dir, not B's. So we
        // test by putting install_b_bin in the bypass as well (simulating
        // cumulative append).
        let bypass =
