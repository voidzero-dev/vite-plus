...
fn run_picker(command_order: &[usize]) -> io::Result<Option<PickedCommand>> {
    ...
    let is_warp = vp_shared::header::is_warp_terminal();
...
fn render_picker(
    ...
    let pad = if vp_shared::header::is_warp_terminal() { " " } else { "" };
    ...
    if vp_shared::header::is_warp_terminal() {
        execute!(stdout, Print(NEWLINE))?;
    }
    execute!(
        stdout,
        Print(format!("{pad}{}", vp_shared::header::vite_plus_header())),
        Print(NEWLINE),
        Print(NEWLINE),
