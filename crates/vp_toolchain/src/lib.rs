...
pub const VERSION_SUMMARY_IDS: [&str; 7] =
    ["vite", "rolldown", "vitest", "oxfmt", "oxlint", "oxlint-tsgolint", "tsdown"];
...
pub fn validate_manifest(manifest: &Manifest) -> Result<(), ToolchainError> {
    ...
    for node in &manifest.nodes {
        ...
        for label in std::iter::once(node.id.as_str())
            .chain(std::iter::once(node.name.as_str()))
            .chain(node.aliases.iter().map(Str::as_str))
        {
            if label.is_empty() {
                return Err(ToolchainError::InvalidManifest(vt_str::format!(
            ...
            if let Some(existing_node) = labels.insert(label, node.id.as_str())
...
pub fn filter_manifest<T: AsRef<str>>(
    ...
    for (index, node) in manifest.nodes.iter().enumerate() {
        names.insert(node.id.as_str(), index);
        names.insert(node.name.as_str(), index);
        for alias in &node.aliases {
            names.insert(alias.as_str(), index);
        }
    }
    ...
    for filter in filters {
        ...
        let Some(&matched) = names.get(filter) else {
...
pub fn render_human(report: &Report) -> Str {
...
pub fn why_hint<T: AsRef<str>>(manifest: &Manifest, queries: &[T]) -> Option<Str> {
    ...
    for query in queries {
        ...
        let Some(node) = manifest.nodes.iter().find(|node| {
            node.id == query
                || node.name == query
                || node.aliases.iter().any(|alias| alias == query)
        }) else {
            continue;
        };
...
mod tests {
    ...
    fn vite_filter_keeps_ownership_and_engine_chain_without_unrelated_edges() {
        ...
        assert_eq!(
            ...
            ["vite-plus", "core", "vite", "rolldown", "oxc"]
        );
        assert_eq!(
            filtered
                .edges
                ...
                .collect::<Vec<_>>(),
            [("vite-plus", "core"), ("core", "vite"), ("vite", "rolldown"), ("rolldown", "oxc")]
        );
    }
    ...
    fn multiple_filters_form_a_stable_union() {
        ...
        assert_eq!(
            filtered.nodes.iter().map(|node| node.id.as_str()).collect::<Vec<_>>(),
            ["vite-plus", "core", "vite", "rolldown", "oxc", "vitest"]
        );
    }
    ...
    fn validation_rejects_filter_labels_shared_by_different_nodes() {
        let mut manifest = manifest();
        manifest.nodes[5].aliases.push("vite".into());
