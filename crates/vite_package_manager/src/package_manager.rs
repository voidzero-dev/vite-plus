...
impl PackageManager {
    pub fn get_bin_prefix(&self) -> PathBuf {
        self.install_dir.join(format!("{}/bin", self.bin_name))
    }
}
...
pub async fn detect_package_manager(cwd: impl AsRef<Path>) -> Result<PackageManager, Error> {
    detect_package_manager_with_default(cwd, None).await
}

/// Detect the package manager from the current working directory with the default package manager type.
pub async fn detect_package_manager_with_default(
    cwd: impl AsRef<Path>,
    default: Option<PackageManagerType>,
    ...
    if version == "latest" {
        // TODO: yarn latest version is v1, should we use v4 by default?
        version = get_latest_version(&package_name).await?;
    ...
    let install_dir =
        download_package_manager(package_manager_type.clone(), &package_name, &version).await?;
...
async fn get_latest_version(package_name: &str) -> Result<CompactString, Error> {
...
async fn download_package_manager(
    ...
    // $CACHE_DIR/vite/package_manager/pnpm/10.0.0/bin/pnpm
    let bin_prefix = target_dir.join(format!("{}/bin", &bin_name));
    ...
    let file_lock = Arc::new(Mutex::new(()));
    {
        ...
        // rename the target directory to make atomic operation
        tracing::debug!("Rename temp dir to {:?}", target_dir);
...
mod tests {
    ...
    async fn test_detect_package_manager_with_pnpm_workspace_yaml() {
        ...
        let result = detect_package_manager(temp_dir.path()).await.expect("Should detect pnpm");
    ...
    async fn test_detect_package_manager_with_yarn_lock() {
        ...
        let result = detect_package_manager(temp_dir.path()).await.expect("Should detect yarn");
    ...
    async fn test_detect_package_manager_with_package_lock_json() {
        ...
        let result = detect_package_manager(temp_dir.path()).await.expect("Should detect npm");
    ...
    async fn test_detect_package_manager_with_package_manager_field() {
        ...
        let result =
            detect_package_manager(temp_dir.path()).await.expect("Should detect pnpm with version");
        assert_eq!(result.bin_name, "pnpm");
    ...
    async fn test_detect_package_manager_with_yarn_package_manager_field() {
        ...
        let result =
            detect_package_manager(temp_dir.path()).await.expect("Should detect yarn with version");
        assert_eq!(result.bin_name, "yarn");
    ...
    async fn test_detect_package_manager_with_npm_package_manager_field() {
        ...
        let result =
            detect_package_manager(temp_dir.path()).await.expect("Should detect npm with version");
        assert_eq!(result.bin_name, "npm");
    }
    ...
    async fn test_detect_package_manager_with_invalid_package_manager_field() {
        ...
        let result = detect_package_manager(temp_dir.path()).await;
        assert!(result.is_err());
    ...
    async fn test_detect_package_manager_with_default_fallback() {
        ...
        let result =
            detect_package_manager_with_default(temp_dir.path(), Some(PackageManagerType::Yarn))
                .await
    ...
    async fn test_detect_package_manager_from_subdirectory() {
        ...
        let result = detect_package_manager(&sub_dir)
            .await
    ...
    async fn test_download_package_manager() {
        ...
        assert!(target_dir.join("yarn/bin/yarn").exists());
        assert!(target_dir.join("yarn/bin/yarn.cmd").exists());
    ...
    async fn test_get_latest_version() {
        let result = get_latest_version("yarn").await;
        assert!(result.is_ok());
