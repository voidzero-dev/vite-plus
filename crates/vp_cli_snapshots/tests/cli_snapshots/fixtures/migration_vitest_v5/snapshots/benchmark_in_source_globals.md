# benchmark_in_source_globals

Recognize local in-source API bindings independently of globals: true, without removing the production guard.

## `vpt write-file vite.config.ts 'export default { test: { globals: true, benchmark: { include: [], includeSource: ['\''perf/*.js'\''] } } };
'`
...
if (import.meta.vitest) {
  const { bench: measure } = import.meta.vitest;
  measure('\''destructured'\'', () => work());
