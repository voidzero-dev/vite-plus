...
Known upstream gap: auto-discovery ignores vite.config.mjs, but explicit loading works. An upstream fix must change this snapshot.

## `vpt cp config.mjs vite.config.mjs`


## `vp lint src`

Automatic discovery should apply the lint block in vite.config.mjs.
...
Automatic discovery should apply the fmt block in vite.config.mjs.
...
## `vp fmt -c vite.config.mjs --check src`
