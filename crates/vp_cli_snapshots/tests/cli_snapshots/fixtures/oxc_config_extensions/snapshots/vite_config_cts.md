...
Known upstream gap: auto-discovery ignores vite.config.cts, but explicit loading works. An upstream fix must change this snapshot.

## `vpt cp config.cjs vite.config.cts`


## `vp lint src`

Automatic discovery should apply the lint block in vite.config.cts.
...
Automatic discovery should apply the fmt block in vite.config.cts.
...
## `vp fmt -c vite.config.cts --check src`
