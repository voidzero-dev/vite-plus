...
## `node -e 'const {execFileSync}=require('\''node:child_process'\'');execFileSync('\''pnpm'\'',['\''exec'\'','\''node'\'','\''-e'\'','\''if(process.version'\!'==process.env.EXPECTED_NODE_VERSION)process.exit(1)'\''],{stdio:'\''inherit'\'',env:{...process.env,EXPECTED_NODE_VERSION:process.version}});console.log('\''pnpm child uses project Node version'\'')'`
