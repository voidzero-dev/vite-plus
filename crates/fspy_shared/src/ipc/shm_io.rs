...
/// Owner of a raw memory region.
pub trait AsRawMemory {
    fn as_raw_slice(&self) -> *mut [u8];
}

impl AsRawMemory for MmapRaw {
    fn as_raw_slice(&self) -> *mut [u8] {
...
impl<M: AsRawMemory> ShmWriter<M> {
...
impl<M: AsRef<[u8]>> ShmReader<M> {
    /// The content of `mem` is assumed to be only written by `ShmWriter`.
...
mod tests {
    ...
    impl AsRawMemory for MockedShm {
        fn as_raw_slice(&self) -> *mut [u8] {
    ...
    // fn shm_with_writer(
    //     size: usize,
    //     writer: impl FnOnce(&mut shm_io::ShmWriter<MockedShm>),
    // ) -> ShmReader<impl AsRef<[u8]>> {
    //     let mem = MockedShm::alloc(size);
    //     let mut shm_writer = unsafe { shm_io::ShmWriter::new(mem.ptr, mem.layout.size()) };
    //     writer(&mut shm_writer);
    //     drop(shm_writer);
    //     ShmReader::new(mem)
    // }
    ...
    fn test_alignment_violation_detection() {
        ...
        impl AsRawMemory for Misaligned {
            fn as_raw_slice(&self) -> *mut [u8] {
    ...
    impl AsRawMemory for shared_memory::Shmem {
        fn as_raw_slice(&self) -> *mut [u8] {
